...
const emailContacto = async (datos) => {
  ...
  //Enviar el email
  await transport.sendMail({
    from: process.env.EMAILI_USER,
    to: 'info.sistemas@qualitybolca.com',
    subject: 'Solicitud de ayuda',
    text: 'Requisicion de compras',
    html: `
            <p>Hola, Tienes una solicitada de ayuda</p>
            <p>
            ${mensaje}
            </p>
        `
...
const emailMejora = async (datos) => {
  ...
  await transport.sendMail({
      ...
      to: datos.email,
      subject: cuerpoCorreoMejora[datos.estatus].asunto + ' ' + datos.nombre_mejora,
  ...
  //     <p>pueden visualizarla entrando a la pagina web de: <a href="www.qualitybolca.net/calidad/evidencias">www.qualitybolca.net/calidad/evidencias</a></p>
  //     `
  //   })
  // }
}
...
const emailMejoraRespuesta = async (datos) => {
  ...
  switch (datos.estatus) {
    case 2:
      resultado = 'En espera de modificacion'
      break;
          case 3:
      resultado = 'Aceptada'
      break;
  ...
  await transport.sendMail({
    from: process.env.EMAILMC_USER,
    to: datos.email,
    subject: 'MEJORA' + ' ' + datos.nombre_mejora + ' fue marcada como' + ' ' + resultado,
    text: 'Prueba de las mejoras',
    ...
    case 2:
      respuestaComite =
        `
          ...
          en la cual se decidió que la propuesta de mejora '<b>${params.nombre_mejora}</b>' se encuentra marcada como <b> MODIFICABLE </b></p>
          <p>para ver el resultado de la evaluación de la mejora puedes acceder desde la pagina WEB <a href="www.qualitybolca.net/admin/mejoracontinua">Pagina Web</a></p>
    ...
    case 3:
      ...
      if(isAfter(params.fecha_respuesta_comite, format(new Date(), 'YYYY-MM-DD'))) {
        respuestaComite =
          ...
          <h5>Buen dia ${params.generador_idea}</h5>
          <p> El día de hoy se llevó a cabo la revisión de las propuestas de mejora continua en la junta con el Comité de Mejora Continua, 
          en la cual se decidió que la propuesta de mejora '<b>${params.nombre_mejora}</b>' se encuentra marcada como <b>ACEPTADA</b></p>
    ...
    case 4:
      respuestaComite =
        `
          <h5>Buen dia ${params.generador_idea}</h5>
          <p> El día de hoy se llevó a cabo la revisión de las propuestas de mejora continua en la junta con el Comité de Mejora Continua, 
          en la cual se decidió que la propuesta de mejora '<b>${params.nombre_mejora}</b>' se encuentra marcada como <b>RECHAZADA</b></p>
          <p>para ver el resultado de la evaluación de la mejora puedes acceder desde la pagina WEB <a href="www.qualitybolca.net/admin/mejoracontinua">Pagina Web</a></p>
          <p>Gracias por enviarnos tu propuesta, te animamos a seguir participando. ¡Saludos! </p>
        `
      break;
...
function generarCuerpoEmail(datos) {
  ...
  let cuerpoCorreo = 'Buen dia ' + datos.generador_idea + ' ' + cuerpoCorreoMejora[datos.estatus].cuerpo1 + ' ' + datos.periodo + ' ' + '<b>' + datos.nombre_mejora + '</b>' + ' ' + cuerpoCorreoMejora[datos.estatus].cuerpo2 + ' ' + cuerpoCorreoMejora[datos.estatus].cuerpo3 + ' ' + cuerpoCorreoMejora[datos.estatus].cuerpo4
